...
using TelcoService.DTO;
using TelcoService.Models;

namespace TelcoService.Controllers;
[Route("api/[controller]")]
[ApiController]
public class ServiceController : Controller
{
    private readonly TelcoServiceContext _context;
    
    public ServiceController(TelcoServiceContext context)
    {
        _context = context;
    }
    ...
    {
        var voiceService = await _context.VoiceService.Where(vs => vs.UserId == userId).FirstOrDefaultAsync();
        var dataService = await _context.DataService.Where(ds => ds.UserId == userId).FirstOrDefaultAsync();
